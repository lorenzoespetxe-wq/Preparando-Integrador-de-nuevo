...
export function PerfilPage(): JSX.Element {
  ...
  if (direccionesQuery.isError) {
    return <p className="text-red-600">No se pudieron cargar las direcciones.</p>;
  }

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold text-orange-900">Mi Perfil</h1>
        <p className="mt-1 text-sm text-slate-700">Actualiza tus datos y administra tus direcciones de entrega.</p>
      </div>

      <section className="space-y-3 rounded-xl border border-orange-100 bg-white p-4 shadow-sm">
        <h2 className="text-lg font-semibold text-orange-900">Datos personales</h2>
      ...
      <section className="space-y-3 rounded-xl border border-orange-100 bg-white p-4 shadow-sm">
        <div className="flex items-center justify-between gap-2">
          <h2 className="text-lg font-semibold text-orange-900">Direcciones de entrega</h2>
          {editingDireccion ? (
            <button
              type="button"
              onClick={() => {
              ...
              className="rounded border border-orange-200 bg-orange-50 px-3 py-1.5 text-sm text-orange-900 hover:bg-orange-100"
            >
              Nueva dirección
            </button>
          ) : null}
        ...
        <div className="space-y-2 pt-2">
          <h3 className="text-sm font-semibold text-orange-900">Direcciones guardadas</h3>
          {direcciones.length > 0 ? (
            direcciones.map((direccion) => (
              <article key={direccion.id} className="rounded-lg border border-orange-100 bg-orange-50/50 p-3">
                <p className="font-medium text-orange-900">{direccion.alias}</p>
                <p className="text-sm text-slate-700">{direccion.linea1}{direccion.linea2 ? `, ${direccion.linea2}` : ""}</p>
                <p className="text-sm text-slate-700">{direccion.ciudad}, {direccion.provincia} ({direccion.codigo_postal})</p>
                <div className="mt-2 flex items-center gap-2">
                  {direccion.es_principal ? (
                    <span className="rounded-full bg-green-100 px-2 py-1 text-xs font-medium text-green-700">Predeterminada</span>
                  ) : null}
                  <button
                    type="button"
            ...
            <p className="text-sm text-slate-700">No tienes direcciones cargadas.</p>
          )}
        </div>
      </section>
    </div>
