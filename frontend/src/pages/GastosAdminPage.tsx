...
export function GastosAdminPage(): JSX.Element {
  ...
  if (ingredientesQuery.isError) {
    return <p className="text-red-600">No se pudieron cargar los gastos.</p>;
  }
  ...
  return (
    <div className="space-y-5">
      <div>
        <h1 className="text-3xl font-bold text-orange-900">Gastos</h1>
        <p className="mt-1 text-sm text-slate-700">Ajusta costos unitarios e inventario mínimo de ingredientes.</p>
      </div>
      ...
      <div className="space-y-3">
        {ingredientes.map((ingrediente) => {
          ...
          return (
            <article
              key={ingrediente.id}
              className="grid gap-3 rounded-xl border border-orange-100 bg-white p-4 shadow-sm md:grid-cols-[2fr,1fr,1fr,auto]"
            >
              <div>
                <h2 className="font-semibold text-orange-900">{ingrediente.nombre}</h2>
