  ...
  return (
    <div className="space-y-4">
      <div className="mb-6">
        ...
        <p className="mt-2 text-orange-700">Catálogo interno de productos</p>
      </div>
      ...
      {productosQuery.isLoading ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {productos.length === 0 ? (
            <p className="col-span-full text-center text-slate-500">No se encontraron productos.</p>
          ) : (
            productos.map((p: Producto) => (
              <Link
                key={p.id}
                to={`/producto/${p.id}`}
                className="rounded-xl border border-orange-100 bg-white p-5 shadow-sm transition hover:shadow-md hover:border-orange-200"
              >
                ...
                {p.descripcion && (
                  <p className="mt-1 text-sm text-slate-600 line-clamp-2">{p.descripcion}</p>
                )}
                ...
                {p.categoria_nombre && (
                  <p className="mt-2 text-xs text-slate-400">{p.categoria_nombre}</p>
                )}
              </Link>
            ))
          )}
