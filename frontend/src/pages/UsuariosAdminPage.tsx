    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-700">Gestión de usuarios y roles.</p>
        </div>
        <button
          type="button"
        ...
        <div className="space-y-3 rounded-xl border border-orange-100 bg-white p-4 shadow-sm">
          ...
          <div className="grid gap-3 md:grid-cols-2">
            <label className="grid gap-1 text-sm text-slate-700">
              Nombre
              <input type="text" value={form.nombre} onChange={(event) => setForm((prev) => (prev ? { ...prev, nombre: event.target.value } : prev))} className="rounded border border-orange-200 px-3 py-2" />
            </label>
            <label className="grid gap-1 text-sm text-slate-700">
              Apellido
              <input type="text" value={form.apellido} onChange={(event) => setForm((prev) => (prev ? { ...prev, apellido: event.target.value } : prev))} className="rounded border border-orange-200 px-3 py-2" />
            </label>
            <label className="grid gap-1 text-sm text-slate-700 md:col-span-2">
              Celular
              <input type="text" value={form.celular} onChange={(event) => setForm((prev) => (prev ? { ...prev, celular: event.target.value } : prev))} className="rounded border border-orange-200 px-3 py-2" />
            </label>
          ...
          <div className="flex gap-2">
            <button type="button" onClick={() => saveMutation.mutate()} disabled={saveMutation.isPending} className="rounded bg-orange-500 px-4 py-2 text-sm font-medium text-white hover:bg-orange-600 disabled:cursor-not-allowed disabled:opacity-60">
              {saveMutation.isPending ? "Guardando..." : "Guardar cambios"}
            </button>
            <button type="button" onClick={() => { setEditingUser(null); setForm(null); }} className="rounded border border-orange-200 bg-orange-50 px-4 py-2 text-sm font-medium text-orange-900 hover:bg-orange-100">
              Cancelar
            </button>
          </div>
        </div>
      ...
      <Modal isOpen={showCreateModal} title="Crear usuario" onClose={() => setShowCreateModal(false)}>
        <div className="grid gap-3">
          <label className="grid gap-1 text-sm text-slate-700">
            Nombre
            <input type="text" value={createForm.nombre} onChange={(e) => setCreateForm((p) => ({ ...p, nombre: e.target.value }))} className="rounded border border-orange-200 px-3 py-2" />
          </label>
          <label className="grid gap-1 text-sm text-slate-700">
            Apellido
            <input type="text" value={createForm.apellido} onChange={(e) => setCreateForm((p) => ({ ...p, apellido: e.target.value }))} className="rounded border border-orange-200 px-3 py-2" />
          </label>
          <label className="grid gap-1 text-sm text-slate-700">
            Email
            <input type="email" value={createForm.email} onChange={(e) => setCreateForm((p) => ({ ...p, email: e.target.value }))} className="rounded border border-orange-200 px-3 py-2" />
          </label>
          <label className="grid gap-1 text-sm text-slate-700">
            Celular (opcional)
            <input type="text" value={createForm.celular} onChange={(e) => setCreateForm((p) => ({ ...p, celular: e.target.value }))} className="rounded border border-orange-200 px-3 py-2" />
          </label>
          <label className="grid gap-1 text-sm text-slate-700">
            Contraseña
            <input type="password" value={createForm.password} onChange={(e) => setCreateForm((p) => ({ ...p, password: e.target.value }))} className="rounded border border-orange-200 px-3 py-2" />
          </label>
          <button
            type="button"
      ...
      <Modal isOpen={!!rolUser} title="Editar roles" onClose={() => setRolUser(null)}>
        {rolUser ? (
          <div className="space-y-3">
            <p className="text-sm text-slate-700">Roles actuales:</p>
            <div className="flex flex-wrap gap-2">
              {rolUser.roles.map((r) => (
                <span key={r} className="flex items-center gap-1 rounded bg-orange-100 px-3 py-1 text-sm font-medium text-orange-800">
                  {r}
                  <button
                    type="button"
                    onClick={() => {
            ...
            <p className="text-sm text-slate-700">Agregar rol:</p>
            <div className="flex flex-wrap gap-2">
              {ROLES_DISPONIBLES.filter((r) => !rolUser.roles.includes(r)).map((r) => (
                <button
                  ...
                  className="rounded border border-blue-200 bg-blue-50 px-3 py-1 text-sm font-medium text-blue-700 hover:bg-blue-100 disabled:opacity-50"
                >
                  + {r}
                </button>
              ))}
