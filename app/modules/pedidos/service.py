...
class PedidoService:
    ...
    async def cancelar_pedido(
        ...
        with PedidoUnitOfWork(self._session) as uow:
            ...
            if is_client and pedido.estado_codigo not in (STATE_EN_PREP, STATE_ESPERANDO_CLIENTE):
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN,
                    detail="Solo puedes cancelar pedidos en preparación",
                )

            if is_client:
                estados_cancelables = [STATE_EN_PREP, STATE_ESPERANDO_CLIENTE]
            else:
